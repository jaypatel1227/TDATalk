...
\begin{frame}{Analyzing the core-periphery structure of the graph}
    \begin{itemize}
        ...
        \item \[CS(i) = Z \sum_{(\alpha, \beta)} C_i(\alpha, \beta) \times R(\alpha, \beta)\]where $Z$ is a normalization factor to make the maximum $CS$ be 1 and this sum is not actually over all $\alpha,\beta$ but instead over a uniform sampling of a discretization of the unit square
    \end{itemize}
\end{frame}
